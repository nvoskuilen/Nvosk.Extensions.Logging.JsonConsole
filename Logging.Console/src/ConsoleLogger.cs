...
namespace Nvosk.Extensions.Logging.JsonConsole
{
    internal class ConsoleLogger : ILogger
    {
        ...
        public virtual void WriteMessage(LogLevel logLevel, string logName, int eventId, string message, Exception exception)
        {
            ...
            if (format == ConsoleLoggerFormat.Json)
            {
                _queueProcessor.EnqueueMessage(CreateJsonLogMessage(logLevel, logName, eventId, message, exception));
            }
            else
            {
        ...
        private LogMessageEntry CreateJsonLogMessage(LogLevel logLevel, string logName, int eventId, string message, Exception exception)
        {
            // Example:
            // {"time":"12:00:00","level":"info","source":"ConsoleApp.Program","event_id":0,"message":"Request received"}

            var messageTemplate = Options.JsonMessageTemplate ?? new JsonConsoleLoggerMessageTemplate();
            var timestampFormat = Options.TimestampFormat;
            var logLevelString = GetLogLevelString(logLevel);
            ...
            {
                writer.WriteString(messageTemplate.TimeStamp, DateTime.Now.ToString(timestampFormat));
            }
            writer.WriteString(messageTemplate.LogLevel, logLevelString);
            writer.WriteString(messageTemplate.Source, logName);
            writer.WriteNumber(messageTemplate.EventId, eventId);
            // TODO: GetScopeInformation
            if (!string.IsNullOrEmpty(message))
            {
                writer.WriteString(messageTemplate.Message, message);
            }
            if (exception != null)
            {
                writer.WriteString(messageTemplate.Exception, exception.ToString());
            }
